...
MACHINE_NAME                   = '<%= scope.lookupvar("fmw_domain::machine_name") %>'
...
clusters_<%= cluster['id'] %>_members= [<% cluster['members'].each do |clust| %>'<%= clust %>',<% end %>]
<% end %>
...
for CLUSTER in CLUSTERS:
  ...
  for CLUSTER_MEMBER in CLUSTER_MEMBERS:
    print "cluster " + CLUSTER + " member " + CLUSTER_MEMBER
    cd('/')
